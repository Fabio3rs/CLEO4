...
namespace CLEO
{
    ...
    OpcodeResult __stdcall opcode_0A96(CRunningScript *thread)
    {
        DWORD handle;
        *thread >> handle;
        *thread << GetPedPool().GetAtHandle(handle);
        return OR_CONTINUE;
    }
    ...
    OpcodeResult __stdcall opcode_0A97(CRunningScript *thread)
    {
        DWORD handle;
        *thread >> handle;
        *thread << GetVehiclePool().GetAtHandle(handle);
        return OR_CONTINUE;
    }
    ...
    OpcodeResult __stdcall opcode_0A98(CRunningScript *thread)
    {
        DWORD handle;
        *thread >> handle;
        *thread << GetObjectPool().GetAtHandle(handle);
        return OR_CONTINUE;
    }
    ...
    OpcodeResult __stdcall opcode_0AB7(CRunningScript *thread)
    {
        DWORD hVehicle;
        *thread >> hVehicle;
        auto id = reinterpret_cast<CVehicleModelInfo*>(Models[GetVehiclePool().GetAtHandle(hVehicle)->m_wModelIndex])->m_nHandlingId;
    ...
    OpcodeResult __stdcall opcode_0AB8(CRunningScript *thread)
    {
        DWORD hVehicle;
        *thread >> hVehicle;
        *thread << GetVehiclePool().GetAtHandle(hVehicle)->m_nCurrentGear;
        return OR_CONTINUE;
    }
    ...
    OpcodeResult __stdcall opcode_0ABD(CRunningScript *thread)
    {
        DWORD hVehicle;
        *thread >> hVehicle;
        SetScriptCondResult(thread, GetVehiclePool().GetAtHandle(hVehicle)->m_nFlags.bSirenOrAlarm);
        return OR_CONTINUE;
    }
    ...
    OpcodeResult __stdcall opcode_0ABE(CRunningScript *thread)
    {
        DWORD hVehicle;
        *thread >> hVehicle;
        SetScriptCondResult(thread, GetVehiclePool().GetAtHandle(hVehicle)->m_nFlags.bEngineOn);
        return OR_CONTINUE;
    }
    ...
    OpcodeResult __stdcall opcode_0ABF(CRunningScript *thread)
    {
        ...
        *thread >> hVehicle >> state;
        auto veh = GetVehiclePool().GetAtHandle(hVehicle);
    ...
    OpcodeResult __stdcall opcode_0AC3(CRunningScript *thread)
    {
        ...
        *thread >> stream >> handle;
        if (stream) stream->Link(GetObjectPool().GetAtHandle(handle));
        return OR_CONTINUE;
    }
    ...
    OpcodeResult __stdcall opcode_0AC4(CRunningScript *thread)
    {
        ...
        DWORD handle;
        *thread >> stream >> handle;
        if (stream) stream->Link(GetPedPool().GetAtHandle(handle));
        return OR_CONTINUE;
    }
    ...
    OpcodeResult __stdcall opcode_0AC5(CRunningScript *thread)
    {
        ...
        DWORD handle;
        *thread >> stream >> handle;
        if (stream) stream->Link(GetVehiclePool().GetAtHandle(handle));
        return OR_CONTINUE;
    }
